...
wget https://raw.githubusercontent.com/coffeegrind123/debian12-harderning-script/main/install.sh
...
1. **Download Scripts:**
   ...
   curl -O https://raw.githubusercontent.com/coffeegrind123/debian12-harderning-script/main/install.sh
...
2. **Copy scripts to container:**
   ...
   docker cp install.sh debian12-hardening-test:/root/
   docker cp systemd-hardening-script.sh debian12-hardening-test:/root/
   
   # Make scripts executable
   docker exec debian12-hardening-test chmod +x /root/install.sh /root/systemd-hardening-script.sh
   ```
...
4. **Run system hardening:**
   ...
   docker exec debian12-hardening-test /bin/bash -c "cd /root && ./install.sh"
   ```
...
docker exec debian12-dev-test /bin/bash -c "cd /scripts && chmod +x *.sh && ./install.sh"
```
